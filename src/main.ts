...
import { applyWireframe, isWireframeEnabled, setWireframeEnabled } from "./core/wireframe";
...
async function boot(): Promise<void> {
  ...
  const manualCamera = new ManualCameraControls(camera.camera, renderer.domElement);
  ...
  const crtEffect = effectManager.effects.find((e): e is CrtEffect => e instanceof CrtEffect);
  const megaFx = effectManager.effects.find((e): e is MegaFxEffect => e instanceof MegaFxEffect);
  ...
  const loop = (): void => {
    ...
    if (reconcileSurface()) {
      if (!megaFx?.renderFrame() && !crtEffect?.renderFrame() && !matrixGrade?.renderFrame()) {
  ...
  function applyShockFrontsToEnemies(): void {
    ...
    for (const front of fronts) {
      ...
      for (const enemy of candidates) {
        ...
        if (enemy.dead) {
          enemy.body.applyLinearImpulseToCenter(nx * impulse, SHOCK_WAVE_LIFT * s, nz * impulse);
  ...
  if (import.meta.env.DEV) {
    ...
    if (new URLSearchParams(location.search).has("record")) {
      const stagePresser = (forward: number, right: number): void => {
        if (scenario instanceof MegaHordeScenario || scenario instanceof RevolutionsScenario) {
        ...
        m.recordMegaShowcase({
          ...
          closestEnemyDistance: () =>
            scenario.enemies.reduce(
              (nearest, enemy) =>
                enemy.dead ? nearest : Math.min(nearest, enemy.distanceTo(player.position)),
              Infinity,
            ),
          hearts: () => ({ current: player.health.current, max: player.health.max }),
          spawnDuelist: () => stagePresser(2.6, 0),
          stagePresser,
          sound: effectManager.effects.find((e): e is SoundEffect => e instanceof SoundEffect),
